 ...
 *   4. Neither               → falls back to mock with a warning
 *
  ...
  async check(
    ...
    if (openAiKey) {
      try {
        ...
        console.warn('[ScopeGuard] OpenAI call failed, falling back to mock:', err);
        return mockCheck(declaredScope, toolCall);
      }
    }
    ...
    if (anthropicKey) {
      try {
        ...
        console.warn('[ScopeGuard] Anthropic call failed, falling back to mock:', err);
        return mockCheck(declaredScope, toolCall);
      }
    }

    console.warn('[ScopeGuard] No LLM API key found; using mock judge. Set OPENAI_API_KEY or ANTHROPIC_API_KEY for real NLI.');
    return mockCheck(declaredScope, toolCall);
  }
}
